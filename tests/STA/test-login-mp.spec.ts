...
test('login', async ({ page }) => {
  await page.goto('https://merchantportal-linkser.eldar-solutions.com/');
  ...
  await login.loginWithCredentials("pruebaLinkser@yopmail.com","Clave123!");
  ...
  await expect(page).toHaveTitle(/Merchant Portal/);
  const welcome = await page.locator('h1');
  await expect(welcome).toContainText('Hola, Ernesto Ron')
